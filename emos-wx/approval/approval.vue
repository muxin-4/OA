	...
	<view class="page" v-if="checkPermission(['WORKFLOW:APPROVAL'])">
		<view class="meeting" v-if="processType=='meeting'">
			...
			<block v-if="taskId!=null">
				<button class="btn" @tap="approveMeeting('同意')" type="primary">同意</button>
				<button class="btn" @tap="approveMeeting('不同意')" type="warn">不同意</button>
			</block>
		</view>
...
<script>
	export default {
		...
		methods: {
			approveMeeting:function(approval){
				let that=this
				uni.showModal({
					title:"提示信息",
					...
					success:function(resp){
						if(resp.confirm){
							...
							that.ajax(that.url.approveTask,"POST",data,function(resp){
								uni.showToast({
									icon:"success",
									title:"处理完成",
									complete:function(){
