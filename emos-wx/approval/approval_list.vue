		...
		<view class="list" v-if="current == 1">
			<view class="item" v-for="one in list" :key="one.id" @tap="toPage(one.processType, one.id, null)">
				...
				<view class="center">
					...
					<view class="attr">
						本人审批：
						<text :class="{ green: one.result_1 == '同意', red: one.result_1 == '不同意' }">
							{{ one.result_1 }}
						</text>
					</view>
					<view class="attr">
						...
						<text
							v-if="one.result_2 != null"
							:class="{ green: one.result_2 == '同意', red: one.result_2 == '不同意' }"
						>
							{{ one.result_2 }}
						</text>
					</view>
...
export default {
	...
	methods: {
		...
		toPage: function(processType, id, taskId) {
			let url = 'approval?processType=' + processType + '&id=' + id;
